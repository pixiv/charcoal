...
import light from '../../theme/dist/tokens/css-variables.json'
import { TailwindConfig } from 'tailwindcss/tailwind-config'
import { flattenKey, mapDefault, fixmeCalc1px } from './util'

export function createTailwindConfigTokenV2() {
        ...
        return [
          [
            ...
            [
              ...
              { lineHeight: fixmeCalc1px(light.text['line-height'][k]) },
            ],
          ],
        ]
      }
      ...
      return Object.entries(v as Record<string, string>).map(([kk, vv]) => {
        return [
          ...
          [
            fixmeCalc1px(vv),
            // @ts-expect-error k should be keyof line-height
            { lineHeight: fixmeCalc1px(light.text['line-height'][k][kk]) },
          ],
        ]
      })
    })
  ...
  const config: TailwindConfig = {
    theme: {
      ...
      borderRadius: Object.fromEntries(
        Object.entries(light.radius).map(([k, v]) => [k, fixmeCalc1px(v)])
      ),
