...
新トークンへ移行する際に旧トークンも引き続き利用したい場合は、以下のマイグレーション css を適用できます。
部分的にカラーを変更したい場合は、新トークンのカラーを上書きしてください。
...
ライトモードは `:root` に、ダークモードは `:root[data-theme='dark']` のセレクターに対応しています。
`@charcoal-react/dist/index.css` の後に記述すると上書きされます。
