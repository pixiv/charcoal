...
// コンテナの高さは CSS で 22px * 行数 + 18px なので、そこから行数に戻す
function rowsOf(textArea: HTMLTextAreaElement) {
  const { height } = containerOf(textArea).getBoundingClientRect()
  return Math.round((height - 18) / 22)
}
...
describe('TextArea autoHeight', () => {
  ...
  it('keeps text rows independent from showCount and does not resize without autoHeight', async () => {
    ...
    // 文字数表示のぶん 1 行ぶん高い
    expect(rowsOf(withCount)).toBe(rowsOf(withoutCount) + 1)
