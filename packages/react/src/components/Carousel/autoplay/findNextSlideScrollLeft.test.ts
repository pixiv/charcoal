...
describe('findNextSlideScrollLeft: 端の扱い', () => {
  ...
  it('非 loop の align=center は巻き戻し位置も restPosition で計算する（先頭が viewport より広いと 0 直書きでは短く着地する）', () => {
    // 先頭(800px)が clientWidth(600) より広い。restPosition(先頭) = 0 + 400 − 300 = 100。
    const wideFirst = [
      { offsetLeft: 0, offsetWidth: 800 },
      { offsetLeft: 800, offsetWidth: 200 },
    ]
    expect(
      findNextSlideScrollLeft(wideFirst, {
        clientWidth: 600,
        maxScroll: 400,
        align: 'center',
        loop: false,
